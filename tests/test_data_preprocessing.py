...
class TestGenotypeDataParser(unittest.TestCase):
    ...
    def test_parse_invalid_allele_count(self):
        ...
        # Empty list format
        # Now we expect it to be handled as missing, not raising an error
        data_empty_list = {'HLA-A': [[]], 'Age': [30]}
        df_empty_list = pd.DataFrame(data_empty_list)
        parsed_genotypes, _ = parser.parse(df_empty_list)
        self.assertEqual(parsed_genotypes[0][0], ["MISSING", "MISSING"])
...
class TestAlleleTokenizer(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        expected_special_tokens = {"PAD": 0, "UNK": 1, "BOS": 2, "EOS": 3, "MISSING": 4}
...
class MockAlleleTokenizer:
    def __init__(self):
        self.special_tokens = {"PAD": 0, "UNK": 1, "BOS": 2, "EOS": 3, "MISSING": 4}
...
class TestHLADataset(unittest.TestCase):
     ...
     def test_getitem_mismatched_haplotype_length_handled_gracefully(self):
         """Test that __getitem__ handles mismatched haplotype lengths gracefully."""
         # Create data with mismatched haplotype
         genotypes = [[['A*01:01', 'A*02:01'], ['B*07:02', 'B*07:02']]]
         covariates = np.array([[0.1, 0.9, -1.0]], dtype=np.float32)
         # Haplotype string instead of tuple/list (simulating a format mismatch)
         phased_haplotypes = ['A*01:01'] 
         ...
         # The dataset should either skip this sample or provide default values
         item = dataset[0]
         # Verify we got something back (exact behavior may vary by implementation)
